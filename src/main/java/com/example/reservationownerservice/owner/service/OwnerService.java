...
import com.example.reservationownerservice.owner.domain.request.OwnerUpdateRequest;
...
public class OwnerService {
    ...
    public OwnerResponse getById(UUID id){
        ...
        return new OwnerResponse(byId.get().getOwnerId(),byId.get().getEmail(), byId.get().getStatus().name());
    }

    public String getOwner(String email){
        Owner byEmail = ownerRepository.findByEmail(email);

        return byEmail.getStatus().name();
    }

    public void updateOwner(OwnerUpdateRequest request){
        Owner byEmail = ownerRepository.findByEmail(request.getEmail());

        byEmail.setStatus(Status.valueOf(request.getStatus()));
